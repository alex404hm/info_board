...
function Sidebar({
  ...
  const floatOrInset = variant === "floating" || variant === "inset"

  // Compute target widths for framer-motion
  let gapWidth: string
  let containerWidth: string
  let containerX: string

  if (collapsible === "offcanvas") {
    gapWidth = isCollapsed ? "0rem" : SIDEBAR_WIDTH
    containerWidth = SIDEBAR_WIDTH
    containerX = isCollapsed ? (side === "left" ? "-100%" : "100%") : "0%"
  } else if (collapsible === "icon") {
    gapWidth = isCollapsed
      ? (floatOrInset ? `calc(${SIDEBAR_WIDTH_ICON} + 1rem)` : SIDEBAR_WIDTH_ICON)
      : SIDEBAR_WIDTH
    containerWidth = isCollapsed
      ? (floatOrInset ? `calc(${SIDEBAR_WIDTH_ICON} + 1rem + 2px)` : SIDEBAR_WIDTH_ICON)
      : SIDEBAR_WIDTH
    containerX = "0%"
  } else {
    gapWidth = SIDEBAR_WIDTH
    containerWidth = SIDEBAR_WIDTH
    containerX = "0%"
  }
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        transition={sidebarTransition}
        className={cn(
          "fixed inset-y-0 z-10 flex h-svh overflow-hidden",
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 flex w-4 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:inset-s-1/2 after:w-0.5 hover:after:bg-sidebar-border ltr:-translate-x-1/2 rtl:-translate-x-1/2",
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex min-w-0 w-full flex-1 flex-col overflow-x-hidden bg-background peer-data-[variant=inset]:m-2 peer-data-[variant=inset]:ml-0 peer-data-[variant=inset]:rounded-xl peer-data-[variant=inset]:shadow-sm peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2",
        className
      )}
      {...props}
    />
