...
export function CalendarPanel() {
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[1.35fr_460px]">
      {/* Calendar grid */}
      <div className="ib-panel p-6 lg:p-7">
        <div className="mb-5 flex items-center justify-between">
          <p className="text-sm font-semibold uppercase tracking-wide text-white/55">Outlook kalender</p>
        ...
        <div className="mb-5 flex items-center justify-between">
          <button onClick={prev} className="flex h-10 w-10 items-center justify-center rounded-lg text-white/40 hover:bg-white/[0.06] active:scale-95">
            <ChevronLeft className="h-4 w-4" />
          </button>
          <div className="flex items-center gap-3">
            <span className="text-base font-bold text-white/90">{MONTHS[vm]} {vy}</span>
            <button onClick={today} className="rounded-full bg-emerald-500/15 px-3 py-1.5 text-[11px] font-semibold text-emerald-400 active:scale-95">
              I dag
            </button>
          </div>
          <button onClick={next} className="flex h-10 w-10 items-center justify-center rounded-lg text-white/40 hover:bg-white/[0.06] active:scale-95">
        ...
        <div className="mb-2 grid grid-cols-7">
          {DAYS.map((d) => (
            <div key={d} className="py-2 text-center text-[11px] font-semibold uppercase tracking-wider text-white/35">{d}</div>
          ))}
        </div>
          ...
          {grid.map((day, i) => {
            ...
            return (
              <button
                ...
                onClick={() => pick(day)}
                className={cn(
                  "flex h-14 flex-col items-center justify-start rounded-lg pt-2.5 text-sm transition-all",
